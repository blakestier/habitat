...
use config::{DispatcherCfg, RouterCfg, ToAddrString};
...
pub trait ApplicationState: Default + Clone + Send {
    fn is_initialized(&self) -> bool;
}

pub struct Application<C: DispatcherCfg + RouterCfg> {
    config: C,
...
where
    C: DispatcherCfg + RouterCfg,
{
    fn new(config: C) -> Result<Self> {
    ...
    fn run<T>(mut self, state: T::State) -> Result<()>
    where
        T: Dispatcher,
    {
...
pub fn start<T, C>(cfg: C, state: T::State) -> Result<()>
where
    T: Dispatcher,
    C: DispatcherCfg + RouterCfg,
{
